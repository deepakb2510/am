...
//	private static final String KEY_STUDENT_PASSWORD = "student_password";
	...
	private static final String KEY_ATTENDANCE_SUBJECT = "attendance_subject";
...
//				KEY_STUDENT_ADDRESS + " TEXT," +
//				KEY_STUDENT_DEPARTMENT + " TEXT," +
//				KEY_STUDENT_CLASS + " TEXT, " +KEY_STUDENT_USERNAME + " TEXT," +
//				KEY_STUDENT_PASSWORD + " TEXT " + ")";
//		Log.d("queryStudent",queryStudent );
	...
	public String StudentAttendance(String studentid,String subject){
		SQLiteDatabase db = this.getWritableDatabase();

		String sql = "Select Count(attendance_status) from attendance_table where attendance_student_id="+studentid+" and attendance_subject='"+subject+"'";
		Cursor c = db.rawQuery(sql,null);
		String result = " ";
		if (c.moveToFirst()){
			result = c.getString(0);
		}
		Log.v("Attendace COunt",result);
		return result;
	}
	public String StudentPresentAttendance(String studentid,String subject){
		SQLiteDatabase db = this.getWritableDatabase();

		String sql = "Select Count(attendance_status) from attendance_table where attendance_student_id="+studentid+" and attendance_status='P' and attendance_subject='"+subject+"'";
		Cursor c = db.rawQuery(sql,null);
		String result = " ";
		if (c.moveToFirst()){
			result = c.getString(0);
		}
		Log.v("Attendace Pres COunt",result);
		return result;
	}
	...
	public void addStudent(StudentBean studentBean) {
		...
		String query = "INSERT INTO student_table (student_firstname,student_lastname,student_mobilenumber,student_address,student_department,student_class, student_username, student_password) values ('"+
				...
				studentBean.getStudent_username()+"', '"+
				studentBean.getStudent_password()+"')";

//		contentValues.put(KEY_STUDENT_FIRSTNAME,studentBean.getStudent_firstname());
//		contentValues.put(KEY_STUDENT_LASTNAME,studentBean.getStudent_lastname());
//		contentValues.put(KEY_STUDENT_MO_NO,studentBean.getStudent_mobilenumber());
//		contentValues.put(KEY_STUDENT_ADDRESS,studentBean.getStudent_address());
//		contentValues.put(KEY_STUDENT_DEPARTMENT,studentBean.getStudent_department());
//		contentValues.put(KEY_STUDENT_CLASS,studentBean.getStudent_class());
//		contentValues.put(KEY_STUDENT_USERNAME,studentBean.getStudent_username());
//		contentValues.put(KEY_STUDENT_PASSWORD,studentBean.getStudent_password());
		...
		//Log.d("query", query);
		try{
//			db.insert(STUDENT_INFO_TABLE,null,contentValues);
//			Log.v("InsetrMssg","Sucess");

			Log.d("query", query);
			db.execSQL(query);
		}catch (SQLException e){
			Log.v("InsertMssg",e.getMessage().toString());
		}
	...
	public StudentBean validateStudent(String userName, String password)
	{
		...
		String query = "SELECT * FROM student_table where student_username='"+userName+"' and student_password='"+password+"'";
		Cursor cursor = db.rawQuery(query, null);
		...
		if(cursor.moveToFirst())
		{
			...
			studentBean.setStudent_username(cursor.getString(7));
			studentBean.setStudent_password(cursor.getString(8));
			return studentBean;
		}
		return null;
	}
...
//		try{
//			/*where student_department='"+branch+"' and student_class='"+year+"'"*/
//			Cursor cursor = db.rawQuery("SELECT * From "+STUDENT_INFO_TABLE+"",null);
//
//				while(cursor.moveToNext()){
//					StudentBean studentBean = new StudentBean();
//					studentBean.setStudent_id(Integer.parseInt(cursor.getString(0)));
...
//				}
//
//			Log.v("DisplayData","Sucess");
//		}catch (Exception e){
//			Log.v("DisplayData",e.getMessage().toString());
//		}
	...
	public void addNewAttendance(AttendanceBean attendanceBean) {
		...
		String query = "INSERT INTO attendance_table values ("+ 
				...
				attendanceBean.getAttendance_subject()+"')";
		Log.d("query", query);
		db.execSQL(query);
		db.close();
	}
